...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Expected output cells: " << output_cells << std::endl;
    ...
    // Hardware emits one 128-bit beat per CaloCell (all fields packed); beat sits in [127:0] of the 512-bit bus
    const size_t out_buf_bytes  = output_cells       * STREAM_WIDTH_BYTES;
    ...
    // -------------------------------------------------------------------
    // Unpack output: each 512-bit beat holds one 128-bit CaloCell in [127:0]
    // Memory layout (little-endian): [0:3]=quality&provenance, [4:7]=time,
    //                                [8:11]=energy, [12:15]=gain_id
    // -------------------------------------------------------------------
    std::vector<CaloCell> results(output_cells);
    for (int i = 0; i < output_cells; i++) {
        const uint8_t* beat = out_buf + i * STREAM_WIDTH_BYTES;
        memcpy(&results[i].gain_and_online_ID, beat + 12, sizeof(int));
        memcpy(&results[i].energy,             beat + 8,  sizeof(float));
        memcpy(&results[i].time,               beat + 4,  sizeof(float));
        memcpy(&results[i].q_and_p,            beat + 0,  sizeof(int));
    }
